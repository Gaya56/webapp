  ...
  const items = [
    {
      from: "system",
      value:
        '<div><small><i>This conversation is shared from <a href="https://combochat.ai"><b>Combochat</b></a></i></small></div>',
    },
  ];
...
export async function uploadToShareGPT(messages = []) {
  const items = await buildItems(messages);

  const resp = await fetch("https://sharegpt.com/api/conversations", {
    method: "POST",
    body: {
